...
class LSTMContext(nn.Module):
    ...
    # def obj_ctx(self, obj_feas, original_fea, subgraph):
        ...
        arr_pad = pad_sequence(self.arr, batch_first=True) # 三维tensor (7600, 72, D) [183, 30, 1703]
        ...
        new_input = torch.cat((obj_new_feature, obj_feas[feaid]), 1) #将新特征和旧特征拼接，作为第二个LSTM的输入
        ...
        return obj_new_feature, node_hidden_feature, c_out #obj_new_feature是双向LSTM层和一层linear层后输出的节点的特征，output_lstm再经过一层LSTM输出的节点特征，c_out是取均值后的特征
